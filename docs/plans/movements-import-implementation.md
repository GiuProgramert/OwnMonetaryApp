...
- [x] **0.1** `alter table movements add column external_id text` + el índice único. Aplicado
  directamente en Supabase por el usuario antes de esta implementación.
- [x] **0.2** Documentado en [`docs/database.md`](../database.md#índices-y-restricciones), sección
  nueva "Índices y restricciones".
...
- [x] **1.1** `lib/imports/types.ts` con el contrato completo.
- [x] **1.2** `helpers/date.ts`, `helpers/amount.ts`, `helpers/headers.ts`. La parte más aburrida y
...
- [x] **1.3** `helpers/fingerprint.ts` con `buildExternalId(row, occurrenceIndex)` y
  `computeExternalIds(rows)` (ver nota de Fase 4.1 — el sufijo de ocurrencia terminó aplicando
  también a `doc:`, no solo a `fp:`).
- [x] **1.4** `readers/xlsx.ts` con import dinámico de `xlsx`. `cellDates: true` normaliza las
  fechas de SheetJS a `Date`; el string `dd/MM/yyyy` (caso real de Itaú, todo el sheet viene como
  texto) y el serial numérico los normaliza `helpers/date.ts`.
- [x] **1.5** `adapters/index.ts` con el registro. Se saltó el adaptador "de ejemplo" armado a mano:
  como ya había un XLSX real (Itaú), fue directo el adaptador real de la Fase 4.

### Fase 2 — Servicios

- [x] **2.1** `getExistingExternalIds(accountId, ids)` en `lib/services/movements.ts` (server),
  scopeado por `accounts.user_id`. Expuesta como Server Action inline (`"use server"` dentro de la
  función, no en el archivo) para que el wizard cliente la pueda invocar sin Route Handler.
- [x] **2.2** `bulkCreateMovements(rows)` en `lib/services/movements.client.ts`: valida cada fila,
  corta en lotes de 200, hace el `upsert` con `ignoreDuplicates`, y devuelve `{ inserted, skipped }`.
- [x] **2.3** `importedMovementSchema = movementSchema.extend({ external_id: z.string().min(1) })`
...
- [x] **3.1** Paso 1 — cuenta destino (`AccountSelect`), banco/formato (select alimentado por
  `ADAPTERS`) y file input.
- [x] **3.2** Pantalla de error de formato, cuando `assertFormat` devuelve `ok: false`: qué columna
  falta y qué columnas se encontraron (`import-format-error.tsx`).
- [x] **3.3** Paso 2 — tabla de preview reusando `Table` de `components/ui/`. Columnas: incluir ·
  estado · fecha · descripción · monto · naturaleza (`Badge`) · tipo de movimiento
  (`MovementTypeSelect` por fila).
- [x] **3.4** Estados por fila: `nueva` · `ya importada` · `duplicada en el archivo` ·
  `error de lectura`. Solo `nueva` viene tildada por defecto.
- [x] **3.5** Acción masiva "asignar tipo a las filas seleccionadas".
- [x] **3.6** Resumen (`import-summary.tsx`, reusado en preview y confirmación): cantidad, total
  crédito, total débito, rango de fechas.
- [x] **3.7** Paso 3 — confirmar, toast con `{ inserted, skipped }`, y
  `revalidateMyDataAndRedirect("/protected/movements")`. El `try/catch` envuelve solo la llamada a
  `bulkCreateMovements`, no el `revalidate...`.
- [x] **3.8** Botón "Importar" en `app/protected/movements/page.tsx`, al lado de "Nuevo".

### Fase 4 — Primer adaptador real (Excel)

- [x] **4.1** Adaptador `itau-cuenta-xlsx` escrito contra `docs/export-example-itau.xlsx`.
  Verificado numéricamente: recalcular el saldo fila por fila desde "Saldo Anterior" con los montos
  extraídos da exactamente el saldo final impreso en el extracto (`48.804.782,00`), 0 filas en
  `issues` sobre 74 filas de datos.

  ⚠️ **Hallazgo que cambia el diseño de dedup:** el nro. de comprobante (`Movimiento`) **no es único
  dentro de un mismo archivo** — se repite para líneas relacionadas con montos distintos (p. ej. un
  cargo y su línea de IVA, mismo `Movimiento`, dos montos). La huella `doc:<id>` a secas hubiera
  colisionado esas filas y perdido una transacción real contra el índice único. `computeExternalIds`
  ahora numera ocurrencias también para `doc:` cuando el identificador se repite en el archivo (antes
  esto solo aplicaba al fallback `fp:`). Ver [`docs/imports.md`](../imports.md#el-identificador-del-banco-no-siempre-es-único-dentro-del-archivo).

  La estabilidad *entre descargas* del mismo comprobante (la pregunta original de "Riesgos", abajo)
...
- [x] **7.1** [`docs/imports.md`](../imports.md): cómo agregar un banco nuevo, los cuatro lugares
  donde se suelen romper los adaptadores, y el hallazgo de la Fase 4.1 sobre `doc:` no único dentro
  del archivo.
- [x] **7.2** Sección "Bank statement imports" en `CLAUDE.md` apuntando a `docs/imports.md`.
...
- ~~**Estabilidad del identificador del banco.**~~ **Confirmado (2026-08-15):** el nro. de
  ...
  > **Matiz agregado en Fase 4.1 (2026-08-15):** estable *entre descargas*, pero **no único
  > *dentro* de un mismo archivo** — Itaú repite el mismo nro. de comprobante en líneas relacionadas
  > (cargo + su IVA). `computeExternalIds` numera ocurrencias para `doc:` cuando esto pasa, igual que
